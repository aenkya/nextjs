      ...
      {isMenuMounted && (
        <ul className={cn(styles.menu, 'flex flex-col absolute bg-gray-100 dark:bg-gray-900', isMenuRendered && styles.menuRendered)}>
          {routes.map(({ name, path, transitionDelay }) => (
            <li
              className="border-b border-gray-300 dark:border-gray-700 text-gray-900 dark:text-beige-100 text-sm font-semibold"
          ...
          >
            <a target="_blank" rel="noopener noreferrer" href="https://enkya.org/BruceBigirwenkyaResume.pdf" className="flex w-auto pb-4">
              Resume
            </a>
          </li>
        </ul>
...
function MenuIcon(props: JSX.IntrinsicElements['svg']) {
  return (
    <svg className="h-5 w-5 absolute text-gray-900 dark:text-beige-100" width="20" height="20" viewBox="0 0 20 20" fill="none" {...props}>
...
function CrossIcon(props: JSX.IntrinsicElements['svg']) {
  return (
    <svg
      className="h-5 w-5 absolute text-gray-900 dark:text-beige-100"
      viewBox="0 0 24 24"
      width="24"
      height="24"
      stroke="currentColor"
